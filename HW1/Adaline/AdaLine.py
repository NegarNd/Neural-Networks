...
def activion_function(input, weights):
    net=0.0
    length = len(input)
...
def find_weights(dataset, target, rate, th):
    ...
    while 1:
        ...
        prediction = activion_function(row, weight)
        # error = prediction - row_target
        for i in range(len(row)):
            weight[i] = weight[i] + rate*(row_target - prediction)*row[i]

        cost_function = 0.5*(row_target - prediction)*(row_target - prediction)
        ...
        if iter_counter == len(dataset):
            cost_function_list.append(cost_function_sum)
            break
    print('cost_finction is: ' , cost_function_list)
